        ...
        Modal.showCustom({
            ...
            body: `
                <div class="connections-modal-wrapper">
                    <div class="dropdown__mini-list" style="display: block; max-height: 400px; overflow-y: auto;">
                        ${modalBody}
                    </div>
                </div>
            `,
...
function renderConnectionsHtml(asset) {
    if (!asset.branches || asset.branches.length === 0) {
        return '<p style="color:#888; padding: 15px; text-align:center;">No branches connected.</p>';
    }

    return asset.branches.map(branch => {
        ...
        if (branchServices.length > 0) {
            servicesHtml = branchServices.map(service => `
                <div class="branch-item" style="display:flex; align-items:center; gap:10px; padding: 6px 15px;">
                    <i class="fa-solid fa-circle" style="color:${c.dot}; font-size:6px;"></i>
                    <span style="font-size: 13px; color: #555;">${service.name}</span>
                </div>
            `).join('');
        } else {
            servicesHtml = `<p style="font-size:12px; color:#bbb; padding-left:25px; margin: 5px 0 10px;">No services</p>`;
        }

        return `
            <div class="service-item" style="background:${c.bg}; border-left:3px solid ${c.border}; border-radius:0 6px 6px 0; margin-top: 10px; padding: 10px;">
                <span style="color:${c.text}; font-weight:600;">${branch.name}</span>
            </div>
            ${servicesHtml}
        `;
    }).join('');
