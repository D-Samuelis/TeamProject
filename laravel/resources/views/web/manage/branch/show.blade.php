...
<script>
    window.BE_DATA = {
        ...
        routes: {
            {{-- Keep consistent key names with the index page so modals
                 (editBranchModal, archiveBranchModal) work on both pages. --}}
    ...
    <aside class="business__sidebar">
        ...
        <section class="business__filters">
            <h3 class="miniLists__subtitle">
                <i class="fa-solid fa-chevron-down"></i> Branch Info
            </h3>
            <div id="branchInfo" class="dropdown__mini-list">
                <div class="business-info-card">
                    <p class="business-info-card__name">{{ $branch->name }}</p>
                    <div class="business-info-card__address"
                         style="font-size:13px;color:var(--color-text-light);margin-bottom:12px;">
            ...
            <header class="business__header-wrapper">
                <div class="business__header-corner">
                    <div class="view-switcher">
                        <button class="view-switcher__btn active">
                            <i class="fa-solid fa-layer-group"></i> Resources
                        </button>
                    </div>
                </div>
                ...
                <div class="business__header-right">
                    <div class="business__header-right-section_1">
                        {{-- Branch dropdown: data-id and data-name are required by archiveBranchModal --}}
                        @if (!$branch->trashed())
                            <div class="dropdown branch-dropdown">
                                ...
                                <div class="branch-dropdown__menu">
                                    <button type="button"
                                            class="branch-dropdown__item delete-action"
                ...
                <section class="service-assigner-panel">
                    ...
                    @php
                        $availableServices = $services->filter(
                            fn($s) => !$branch->services->contains($s->id)
                        );
                    @endphp

                    <div id="assign-container">
                        <div id="multiselect-wrapper"
                             style="{{ $availableServices->count() > 0 ? '' : 'display:none;' }}">
                        ...
                        <div id="empty-select-message" class="select-empty-message"
                             style="{{ $availableServices->count() > 0 ? 'display:none;' : '' }}">
