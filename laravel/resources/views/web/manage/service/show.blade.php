...
@section('content')
    @php
        $serviceJsData = [
            ...
            'cancellation_period' => App\Application\Service\Services\DurationParser::fromMinutes(
                $service->cancellation_period_minutes,
            ),
            ...
            'business' => $service->business
                ? ['id' => $service->business->id, 'name' => $service->business->name]
                : null,
            ...
            toolbar: {
                ...
                centerGroups: [{
                        ...
                        actions: [{
                            ...
                            icon: @json($service->is_active) ? 'fa-circle text-green' :
...
<<<<<<< HEAD
                            ...
                            toastTitle: @json($service->is_active) ? 'Service deactivated' :
                                'Service activated',
                            toastType: @json($service->is_active) ? 'warning' : 'success',
                            toastText: @json($service->is_active) ?
                            ...
                            @if ($service->trashed())
                                {
                                    ...
                                    icon: 'fa-rotate-left',
                                    isForm: true,
                                    toastTitle: 'Service restored',
                                    toastType: 'success',
                                    toastText: 'The service is now active again.',
                                    action: '{{ route('manage.service.restore', $service->id) }}',
                                    hiddenFields: [{
                                        name: '_method',
                                        value: 'PATCH'
                                    }]
                            ...
                            @else
                                {
                                    ...
                                    toastTitle: 'Service archived',
                                    toastType: 'warning',
                                    toastText: 'The service has been archived.'
                                }
                            @endif
                        ]
                    }
            ...
            <section class="business__filters">
                ...
                <div id="serviceInfo" class="dropdown__mini-list">
                    <div class="business-info-card">
                        ...
                        @can('update', $service)
                            @if (!$service->trashed())
                                <button class="business-info-card__edit-btn" type="button"
                                ...
                                <div class="dropdown branch-dropdown">
                                    ...
                                    <div class="branch-dropdown__menu">
                                        <form action="{{ route('manage.service.update', $service->id) }}" method="POST">
                                            ...
                                            <input type="hidden" name="cancellation_period"
                                                value="{{ $currentCancellation }}">
                                            <input type="hidden" name="requires_manual_acceptance"
                                                value="{{ $service->requires_manual_acceptance ? 1 : 0 }}">
                                            <input type="hidden" name="is_active"
                                                value="{{ $service->is_active ? 0 : 1 }}">
                                            <button type="submit" class="branch-dropdown__item">
                                                <i
                                                    class="fa-solid fa-circle status-dot {{ $service->is_active ? 'text-green' : 'text-yellow' }}"></i>
                                                Status:
                                                <div
                                                    class="status__badge {{ $service->is_active ? 'bg__badge-green' : 'bg__badge-yellow' }}">
                                        ...
                                        <form action="{{ route('manage.service.update', $service->id) }}" method="POST">
                                            ...
                                            @if ($service->location_type)
                                                <input type="hidden" name="location_type"
                                                    value="{{ $service->location_type }}">
                                            @endif
                                            <input type="hidden" name="cancellation_period"
                                                value="{{ $currentCancellation }}">
                                            <input type="hidden" name="is_active"
                                                value="{{ $service->is_active ? 1 : 0 }}">
                                            <input type="hidden" name="requires_manual_acceptance"
                                                value="{{ $service->requires_manual_acceptance ? 0 : 1 }}">
                                            <button type="submit" class="branch-dropdown__item">
                                                <i class="fa-solid fa-user-check"></i>
                                                Acceptance:
                                                <div
                                                    class="status__badge {{ $service->requires_manual_acceptance ? 'bg__badge-yellow' : 'bg__badge-green' }}">
                                        ...
                                        @can('delete', $service)
                                            <button type="button" class="branch-dropdown__item delete-action"
                                                data-modal-target="archive-service-modal" data-id="{{ $service->id }}"
                            ...
                            <div class="service-settings__grid">
                                <section class="service-settings__connections">
                                    <div class="service-settings__inline-card">
                                        <div
                                            class="service-settings__inline-card-head service-settings__inline-card-head--readonly">
                                            <div>
                                                <span class="service-settings__fact-label">Assigned Branches</span>
                                                <strong>{{ $service->branches->count() }}</strong>
                                                <p class="service-settings__helper-text">
                                                    Assigned branches define where the service can be offered. Use the
                                                    actions below to add or remove this service from a branch.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="service-settings__assigned-branches">
                                        @forelse($branches->where('business_id', $businessId) as $branch)
                                            ...
                                            <div
                                                class="service-settings__assigned-branch {{ $isAssigned ? 'is-assigned' : '' }}">
                                                <a href="{{ route('manage.branch.show', $branch->id) }}"
                                                    class="service-settings__assigned-branch-link">
                                                    <i class="fa-solid fa-location-dot"></i>
                                                    <span>
                                                ...
                                                @can('assign', [$service, $branch])
                                                    @if ($isAssigned)
                                                        <form method="POST"
                                                            action="{{ route('manage.service.branch.unassign', [$service->id, $branch->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                                                class="service-settings__branch-action service-settings__branch-action--remove">
                                                                Remove
                                                            </button>
                                                        </form>
                                                    @else
                                                        <form method="POST"
                                                            action="{{ route('manage.service.branch.assign', [$service->id, $branch->id]) }}">
                                                            @csrf
                                                            <button type="submit"
                                                                class="service-settings__branch-action service-settings__branch-action--assign">
                                                                Assign
                                                            </button>
                                                        </form>
                                                    @endif
                                                        ...
                                                        <div class="service-settings__tree-assets">
                                                            @forelse($branchAssets as $asset)
                                                                <a href="{{ route('manage.asset.show', $asset->id) }}"
                                                                    ...
                                                                    <div>
                                                                        <span
                                                                            class="service-settings__fact-label">Asset</span>
...
>>>>>>> 9b2034c34521c9a6ab3916fb5b482b8336129fbf
                                    ...
                                    @else
                                        <div class="service-settings__tree-empty">
                                            No business linked to this service.
                                        </div>
                                    @endif
                                </div>
                            </div>
    ...
    {{-- Hidden template for toolbar connections dropdown --}}
