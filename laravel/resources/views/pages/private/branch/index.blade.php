...
@forelse($branches as $branch)
    <div style="border:1px solid #ddd;border-radius:6px;padding:1rem;margin-bottom:0.75rem;display:flex;justify-content:space-between;align-items:center;">
        <div>
            <a href="{{ route('branch.show', $branch->id) }}" style="font-weight:500;">{{ $branch->name }}</a>
...
<div id="createBranchModal" class="modal-backdrop" style="display:none;">
    <div class="modal-box">
        ...
        <form method="POST" action="{{ route('branch.store') }}">
            @csrf
            @include('pages.private.branch.partials.branch-form', ['prefix' => 'create', 'branch' => null, 'businesses' => $businesses])
...
@if($errors->any())
<script>openModal('createBranchModal');</script>
@endif
