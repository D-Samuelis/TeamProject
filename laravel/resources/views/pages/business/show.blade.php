...
@section('content')
        ...
        <aside class="business__sidebar">
            ...
            <section class="business__filters">
                ...
                <div id="manageEmployeesList" class="dropdown__mini-list">
                    <button type="button" class="business__nav-link is-active"
                    ...
                    <div class="dashboard-card">
                        ...
                        <div class="card-body">
                            <div class="services-grid">
                                ...
                                {{-- BranchServices per branch --}}
                                @foreach ($business->branches as $branch)
                                    @foreach ($branch->branchServices as $branchService)
                                        <div class="service-item-card filterable-service"
                                            data-belongs-to="branch-{{ $branch->id }}">
                                            <div class="service-item-card__header">
                                                <strong>{{ $branchService->name }}</strong>
                                            </div>
                                            <div class="service-item-card__footer">
                                                <a href="{{ route('manage.branch.show', $branchService->id) }}">Edit
                                                    Schedule</a>
                                            </div>
                                        </div>
                                    @endforeach
                                ...
                                {{-- Business services always shown in summary --}}
                                @foreach ($business->services as $service)
                                    <div class="service-item-card filterable-service" data-belongs-to="all">
                                        <div class="service-item-card__header">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Filtering function
            function applyFilter(filter, branchId, branchName) {
                filterItems.forEach(i => i.classList.remove('active'));
                if (filter === 'all') {
                    const allItem = document.querySelector('#branchesList .branch-filter-item[data-filter="all"]');
                    if (allItem) allItem.classList.add('active');
                } else {
                    filterItems.forEach(i => {
                        if (i.getAttribute('data-branch-id') === branchId) i.classList.add('active');
                    });
                }
                ...
                members.forEach(m => {
                    ...
                    m.style.display = (filter === 'all' || belongs === filter || belongs === 'all') ?
                        'flex' : 'none';
                });
                ...
                services.forEach(s => {
                    ...
                    if (filter === 'all') {
                        s.style.display = belongs === 'all' ? 'block' : 'none';
                    } else {
                        s.style.display = belongs.split(' ').includes(filter) ? 'block' : 'none';
                    }
                });
            }
            ...
            filterItems.forEach(item => {
                item.addEventListener('click', function() {
                    ...
                    applyFilter(filter, branchId, branchName);
                });
            });

            // --- DEFAULT ON PAGE LOAD ---
            const defaultFilterItem = document.querySelector(
            '#branchesList .branch-filter-item[data-filter="all"]');
            if (defaultFilterItem) {
                const branchId = defaultFilterItem.getAttribute('data-branch-id') || '';
                const branchName = defaultFilterItem.querySelector('.member-name')?.innerText || '';
                applyFilter('all', branchId, branchName);
            }
        });
    </script>
    ...
    @php
        function _resolveAssignment($user, $business): array
        {
            ...
            } elseif ($modelName === 'branchservice') {
                $branchService = null;
                foreach ($business->branches as $branch) {
                    $branchService = $branch->branchServices->firstWhere('id', $user->pivot->model_id);
                    if ($branchService) {
                        $displayName = $branchService->name ?? 'Unknown BranchService';
                        break;
                    }
                }
            }
